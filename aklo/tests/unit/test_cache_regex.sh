...
test_statistics_tracking() {
    ...
    # Exporter les stats dans un fichier temporaire
    local stats_file
    stats_file="/tmp/aklo_regex_stats_$$.txt"
    export_regex_cache_stats "$stats_file"
    
    # Lire les valeurs exportées
    local hits
    hits=$(grep '^PBI_ID_hits=' "$stats_file" | cut -d'=' -f2)
    local misses
    misses=$(grep '^NONEXISTENT_miss=' "$stats_file" | cut -d'=' -f2)
